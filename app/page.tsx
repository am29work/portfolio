  ...
  return (
    <div className={darkMode ? "dark" : ""}>
      <main className="bg-amber-50 px-10 dark:bg-gray-900 md:px-20 lg:px-40 pt-10 md:pt-16 transition-all min-h-screen">
              ...
              {projects.map((project) => (
                ...
                >
                  <div className="relative aspect-[16/10] w-full overflow-hidden rounded-2xl shadow-2xl border border-purple-500/10">
                    <Image src={project.image} alt={project.title} fill className="object-cover transition-all duration-700 group-hover/card:scale-110" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-0 group-hover/card:opacity-100 transition-all duration-500 flex flex-col justify-end p-6">
                      <h4 className="text-xl font-bold text-white mb-2 transform translate-y-4 group-hover/card:translate-y-0 transition-transform duration-500">{project.title}</h4>
                      <div className="flex flex-wrap gap-2 mb-4">
                        {project.tech.map((t) => (
                          <span key={t} className="text-[10px] uppercase tracking-wider px-2 py-1 rounded-full bg-yellow-500/20 border border-yellow-500/50 text-yellow-500 font-bold shadow-[0_0_10px_rgba(234,179,8,0.3)]">{t}</span>
                        ))}
                      </div>
                      <p className="text-gray-300 text-sm mb-4 opacity-0 group-hover/card:opacity-100 delay-100 transition-opacity">Click to view full architecture documentation & CI/CD logs.</p>
                    </div>
                  </div>
                </Link>
              ))}
        ...
        {/* Floating Bottom Dock with Liquid Glass effect */}
